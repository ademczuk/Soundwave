...
- ACTIVE direction (2026-06-14): retrieval + eval, NOT model training. Living plan `soundwave-mcp/SOUNDWAVE_NEXT_VERIFIER_HARNESS_PLAN.md` (Option-G, every phase gated on a measured number; weights are plateaued so the lever moved to retrieval). Foundation + instruments shipped in `soundwave-mcp/v8_trackB/` (the grounded recall harness, the multi-label scorer, the skills.cypher enrichment, the tactic crosswalk). First real recall numbers: @1 49% / @3 68% / @5 72% on BM25; command enrichment is an aggregate wash but fixes procedure queries; off-the-shelf rerankers do not help here (left default-off). Next lever is a small dense embedder measured against the harness.
- Future-training recipes (DEFERRED, not the current direction): `soundwave-mcp/SOUNDWAVE_VNEXT.md`, `soundwave-mcp/SOUNDWAVE_GRPO_PLAN.md`, `soundwave-mcp/UNIFIED-RETRAIN-PLAN.md`, `soundwave-mcp/PAID-GATE-PLAN.md`.
